...
# Initialisation des courbes cumulées
Cv_cumul = np.zeros_like(T_vals)
chi_cumul = np.zeros_like(T_vals, dtype=np.complex128)

# Boucle cumulée sur N
for N in range(1, N_max + 1):
    df = pd.read_csv(f"CSV_cluster/clusters_carre_N{N}.csv")
...
# Sauvegarde
results.to_csv(f"CSV_Thermodynamics/thermodynamique_reseau_carre_p{str(p).replace('.', '')}.csv", index=False)
